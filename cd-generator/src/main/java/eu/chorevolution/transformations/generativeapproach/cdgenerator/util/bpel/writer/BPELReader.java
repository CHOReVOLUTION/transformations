...
public class BPELReader implements ErrorHandler {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(BPELReader.class);
	...
	public void read (BPELResource resource, InputStream inputStream)  {
		...
		try {
			...
			LOGGER.error("I/O Error Reading BPEL XML", ioe);
		} finally {			
		}
		if (doc != null) {
			pass1(doc);		
	 ...
	 */
	protected void setPartnerLink(Element activityElement, final EObject eObject,
	 ...
	 */
	protected void setVariable(Element activityElement, final EObject eObject, String variableNameAttr,
			final EReference reference) {
	...
	protected void setEventHandler(Element element, BPELExtensibleElement extensibleElement) {
		...
		if (eventHandlerElements.size() > 0) {
			...
			if (extensibleElement instanceof Process) ((Process)extensibleElement)
				.setEventHandlers(eventHandler);
				else if (extensibleElement instanceof Scope) ((Scope)extensibleElement)
					.setEventHandlers(eventHandler);
		}
	}	
	...
	protected void setOperationParms(final Element activityElement,
		...
		if (activityElement.hasAttribute("operation")) {
            if (portType != null) {
				activity.setOperation(BPELUtils
						.getOperation(getResource().getURI(), portType, activityElement, "operation"));
			} else {
	...
	protected void setOperationParmsOnMessage(final Element activityElement, final OnMessage onMessage) {
        ...
        if (activityElement.hasAttribute("operation")) {
            if (portType != null) {
                onMessage.setOperation(BPELUtils
                		.getOperation(getResource().getURI(), portType, activityElement, "operation"));
            } else {
	...
	protected void setOperationParmsOnEvent(final Element activityElement, final OnEvent onEvent) {
        ...
        if (activityElement.hasAttribute("operation")) {
            if (portType != null) {
                onEvent.setOperation(BPELUtils
                		.getOperation(getResource().getURI(), portType, activityElement, "operation"));
            } else {
	...
	protected Process xml2Process(Element processElement) {
		...
		if (processElement.hasAttribute("suppressJoinFailure"))
			process.setSuppressJoinFailure( Boolean.valueOf( processElement
					.getAttribute("suppressJoinFailure").equals("yes")));
		
		if (processElement.hasAttribute("exitOnStandardFault"))
			process.setExitOnStandardFault( Boolean.valueOf( processElement
					.getAttribute("exitOnStandardFault").equals("yes")));
		
		if (processElement.hasAttribute("variableAccessSerializable"))
			process.setVariableAccessSerializable( Boolean.valueOf( processElement
					.getAttribute("variableAccessSerializable").equals("yes")));
		...
		Element messageExchangesElements = getBPELChildElementByLocalName(processElement,
				"messageExchanges");
	...
	protected Extension xml2Extension(Element extensionElement) {
		...
		if (extensionElement.hasAttribute("mustUnderstand"))
			extension.setMustUnderstand( Boolean.valueOf( extensionElement.getAttribute("mustUnderstand")
					.equals("yes")));
	 ...
	 */
  	protected PartnerLink xml2PartnerLink(Element partnerLinkElement) {
		...
		if (partnerLinkElement.hasAttribute("initializePartnerRole"))
			partnerLink.setInitializePartnerRole( Boolean.valueOf( partnerLinkElement
		...
		if (partnerLinkTypeName != null && partnerLinkTypeName.getSpecified()) {
			...
			if(slt != null) {
				...
				if (partnerLinkElement.hasAttribute("myRole")) {
					RoleProxy role = new RoleProxy(getResource(), slt, partnerLinkElement
							.getAttribute("myRole"));
					partnerLink.setMyRole(role);
				}
				if (partnerLinkElement.hasAttribute("partnerRole")) {
					RoleProxy role = new RoleProxy(getResource(), slt, partnerLinkElement
							.getAttribute("partnerRole"));
	...
	protected Catch xml2Catch(Element catchElement) {
		...
		if (catchElement.hasAttribute("faultVariable")) {
			// Set fault variable
			Variable variable = BPELFactory.eINSTANCE.createVariable();
			// Should not this be the variable proxy ?
	...
	protected Source xml2Source(Element sourceElement) {
		...
		Element transitionConditionElement = getBPELChildElementByLocalName(sourceElement,
				"transitionCondition");
	...
	protected Activity xml2Scope(Element scopeElement) {
		...
		Element terminationHandlerElement = getBPELChildElementByLocalName(scopeElement,
				"terminationHandler");
	...
	protected Activity xml2Flow(Element flowElement) {
		...
		Element completionConditionElement = getBPELChildElementByLocalName(flowElement,
				"completionCondition");
	...
	protected Activity xml2Pick(Element pickElement) {
		...
		if (createInstance != null && createInstance.getSpecified()) 
       		pick.setCreateInstance(Boolean
       				.valueOf(createInstance.getValue().equals("yes") ? "True":"False"));  	
	...
	protected OnMessage xml2OnMessage(Element onMessageElement) {
		...
		// Set messageExchange
		setMessageExchange(onMessageElement, onMessage, BPELPackage.eINSTANCE
	...
	protected Copy xml2Copy(Element copyElement) {
		...
		if (copyElement.hasAttribute("keepSrcElementName"))
			copy.setKeepSrcElementName( Boolean.valueOf( copyElement.getAttribute("keepSrcElementName")
					.equals("yes")));
		
		if (copyElement.hasAttribute("ignoreMissingFromData")) 
			copy.setIgnoreMissingFromData( Boolean.valueOf( copyElement
					.getAttribute("ignoreMissingFromData").equals("yes")));
				...
				if (candidateChild != null && candidateChild.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (deserializer == null &&
						...
						ServiceReferenceDeserializer referenceDeserializer = extensionRegistry
								.getServiceReferenceDeserializer(serviceRef.getReferenceScheme());
						if (referenceDeserializer != null) {
							Object serviceReference = referenceDeserializer.unmarshall(childElement,
									process);
	...
	protected Activity xml2Invoke(Element invokeElement) {
		...
		setOperationParms(invokeElement, invoke, null, BPELPackage.eINSTANCE.getInvoke_InputVariable(),
				BPELPackage.eINSTANCE.getInvoke_OutputVariable(), 
				BPELPackage.eINSTANCE.getPartnerActivity_PartnerLink());
		...
		if (faultHandler != null && (!faultHandler.getCatch().isEmpty() ||
				faultHandler.getCatchAll() != null)) {
	...
	protected Activity xml2Reply(Element replyElement) {
		...
		setOperationParms(replyElement, reply, BPELPackage.eINSTANCE.getReply_Variable(), null, null, 
				BPELPackage.eINSTANCE.getPartnerActivity_PartnerLink());
	...
	protected Activity xml2Receive(Element receiveElement) {
		...
		setOperationParms(receiveElement, receive, BPELPackage.eINSTANCE.getReceive_Variable(), null, null, 
				BPELPackage.eINSTANCE.getPartnerActivity_PartnerLink());
	 ...
	 */
	protected void setMessageExchange(Element activityElement, final EObject eObject,
			final EReference reference) {
		...
		fPass2Runnables.add(new Runnable() {
			public void run() {	
				MessageExchange targetMessageExchange = BPELUtils
						.getMessageExchange(eObject, messageExchangeName);
	...
	protected Activity xml2ForEach(Element forEachElement) {
		...
		Element startCounterValueElement = getBPELChildElementByLocalName(forEachElement,
				"startCounterValue");
		...
		Element finalCounterValueElement = getBPELChildElementByLocalName(forEachElement,
				"finalCounterValue");
		...
		Element completionConditionElement = getBPELChildElementByLocalName(forEachElement,
				"completionCondition");
		if (completionConditionElement != null) {
			CompletionCondition completionCondition = xml2CompletionCondition(completionConditionElement);
	...
	protected Branches xml2Branches(Element branchesElement) {
		...
		if (branchesElement.hasAttribute("successfulBranchesOnly"))
			branches.setCountCompletedBranchesOnly( Boolean.valueOf( branchesElement
					.getAttribute("successfulBranchesOnly").equals("yes")));
	...
	protected Correlation xml2Correlation(Element correlationElement) {
		...
		if (correlationElement.hasAttribute("set")) {
			...
			fPass2Runnables.add(new Runnable() {
				public void run() {	
					CorrelationSet cSet = BPELUtils.getCorrelationSetForActivity(correlation,
							correlationSetName);
					if (cSet == null) {
	...
	protected void xml2ExtensibleElement (BPELExtensibleElement extensibleElement, Element element) {
		...
		for (int i=0, n=nodeMap.getLength(); i<n; i++) {
			...
			if (attr.getNamespaceURI() != null && !attr.getNamespaceURI().equals(XSDConstants
					.XMLNS_URI_2000)) {
				nodes.add(attr);	
			}
		}
	...
	protected void deserialize ( BPELExtensibleElement ee, Attr attr) {
		...
		Element tempElement = attr.getOwnerDocument().createElementNS(attr.getNamespaceURI(),
   ...
   static public boolean isEmptyOrWhitespace( String value ){
	...
	public void fatalError(SAXParseException exception)  {
		...
		// add the error to resource
		if (fCurrentResource!=null)
			fCurrentResource.getErrors().add(new SAXParseDiagnostic(exception, SAXParseDiagnostic
					.FATAL_ERROR));
	}
