...
public interface CDGenerator {
	
	byte[] generateCDWSDL(ProjectionData projectionData);
	byte[] generateCDClientWSDL(ProjectionData projectionData,
			List<TaskCorrelationData> tasksCorrelationData);
	CD generateCDProsumer(List<String> prosumersParticipantsNames,
	...
	CD generateCDClient(List<TaskCorrelationData> tasksCorrelationData, ProjectionData projectionData, 
			List<WSDLData> wsdlData) throws CDGeneratorException;
}
