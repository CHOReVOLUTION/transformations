...
public class CDConsumerPartIndexJSPGenerationTests {
	private static final Logger LOGGER = LoggerFactory.getLogger(CDConsumerPartIndexJSPGenerationTests.class);
	
	private static final String INDEX_JSP_FILE_NAME = "index.jsp";
	...
	@Test
	public void generateIndexJSPCDConsumerPart_01() {

		String indexFilePath = TEST_OUTPUT_RESOURCES + File.separatorChar + INDEX_JSP_FILE_NAME;
		try {
			FileUtils.forceMkdir(new File(TEST_OUTPUT_RESOURCES));
			CDConsumerPartData cdConsumerPartData = new CDConsumerPartData();
			cdConsumerPartData.setName("test-jsp");
			cdConsumerPartData.setArtifactId("test-jsp");
			cdConsumerPartData.setGroupId("eu.chorevolution.testjsp.prosumers");
			cdConsumerPartData.setPackagename("eu.chorevolution.testjsp.prosumers");
			cdConsumerPartData.setWsdlname("testjsp");
			CDConsumerPartGeneratorUtility.generateIndexJspFile(cdConsumerPartData, indexFilePath);
		} catch (IOException e) {
			LOGGER.error("generateIndexJSPCDConsumerPart_01 > ", e);
