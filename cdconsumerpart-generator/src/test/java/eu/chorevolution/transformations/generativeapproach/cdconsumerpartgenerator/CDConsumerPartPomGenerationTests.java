...
public class CDConsumerPartPomGenerationTests {
	private static final Logger LOGGER = LoggerFactory.getLogger(CDConsumerPartPomGenerationTests.class);
	...
	@Test
	public void generatePomCDConsumerPart_01() {

		String pomFilePath = TEST_OUTPUT_RESOURCES + File.separatorChar + POM_XML_FILE_NAME;
		try {
			FileUtils.forceMkdir(new File(TEST_OUTPUT_RESOURCES));
			CDConsumerPartData cdConsumerPartData = new CDConsumerPartData();
			cdConsumerPartData.setName("test-pom");
			cdConsumerPartData.setArtifactId("testpom");
			cdConsumerPartData.setGroupId("eu.chorevolution.testpom.prosumers");
			cdConsumerPartData.setPackagename("eu.chorevolution.testpom.prosumers");
			cdConsumerPartData.setWsdlname("testpom");
			CDConsumerPartGeneratorUtility.generatePomXMLFile(cdConsumerPartData, pomFilePath);
		} catch (IOException e) {
			LOGGER.error("generatePomCDConsumerPart_01 > ", e);
