...
public class CDConsumerPartWebMvcConfigXMLGenerationTests {
	...
	@Test
	public void generateWebMvcXMLCDConsumerPart_01() {

		String webMvcXMLFilePath = TEST_OUTPUT_RESOURCES + File.separatorChar + WEB_MVC_CONFIG_XML_FILE_NAME;
		try {
			FileUtils.forceMkdir(new File(TEST_OUTPUT_RESOURCES));
			CDConsumerPartData cdConsumerPartData = new CDConsumerPartData();
			cdConsumerPartData.setName("test-webmvc");
			cdConsumerPartData.setArtifactId("testwebmvc");
			cdConsumerPartData.setGroupId("eu.chorevolution.testwebmvc.prosumers");
			cdConsumerPartData.setPackagename("eu.chorevolution.testwebmvc.prosumers");
			cdConsumerPartData.setWsdlname("testwebmvc");
			CDConsumerPartGeneratorUtility.generateWebMvcConfigXMLFile(cdConsumerPartData, webMvcXMLFilePath);
		} catch (IOException e) {
			LOGGER.error("generateWebMvcXMLCDConsumerPart_01 > ", e);
