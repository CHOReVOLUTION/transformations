...
public class CDConsumerPartBusinessServiceInterfaceGenerationTests {
	private static final Logger LOGGER = LoggerFactory.getLogger(CDConsumerPartSpringServiceImplGenerationTests.class);
	...
	@Test
	public void generateBusinessServiceInterfaceCDConsumerPart_01() {
		...
		try {
			...
			CDConsumerPartGeneratorUtility.generateBusinessServiceInterface(TEST_OUTPUT_RESOURCES, generatingJavaData);

		} catch (WSDLException | IOException e) {
			e.printStackTrace();
			LOGGER.error("generateBusinessServiceInterfaceCDConsumerPart_01 > ", e);
			Assert.assertTrue(false);
		}
	}
	
	@Test
	public void generateBusinessServiceInterfaceCDConsumerPart_02() {
		//Generated the seadasearp business interface wp4 without loop
		try {
			CDConsumerPartData cdConsumerPartData = new CDConsumerPartData();
			...
			e.printStackTrace();
			LOGGER.error("generateBusinessServiceInterfaceCDConsumerPart_02 > ", e);
			Assert.assertTrue(false);
		}
	}

	@Test
	public void generateBusinessServiceInterfaceCDConsumerPart_03() {
		//Generated the seadaseatsa business interface wp4 WITH loop
		try {
			CDConsumerPartData cdConsumerPartData = new CDConsumerPartData();
			...
			e.printStackTrace();
			LOGGER.error("generateBusinessServiceInterfaceCDConsumerPart_03 > ", e);
			Assert.assertTrue(false);
		}
	}	
